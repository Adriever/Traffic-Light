# Traffic-Light
A program that times 7 LED flashes as a traffic light, with a pedestrian light
Changed timing so that yellow flashes were shorter
Shortened green time when button is pressed
Added pedetrian button and LED, as well as pedestrian button and LED variables
Made pedestrian button turn both lights red and turn pedestrian light on for a timed amount
Made pedestrian light flip whichever green light was initially on once pedestrian light timer is done
Extended pedestrian light time and reduced pedestrian light changing time
Ready to be graded
